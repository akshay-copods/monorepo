...
- `packages/@smallstair/core`: Core React components
- `packages/ts-config-smallstair`: Shared `tsconfig.json`s used throughout the Turborepo
...
For `smallstair-core`, the `build` command is the following:

```bash
tsup src/index.tsx --format esm,cjs --dts --external react
```

`tsup` compiles `src/index.tsx`, which exports all of the components in the design system, into both ES Modules and CommonJS formats as well as their TypeScript types. The `package.json` for `smallstair-core` then instructs the consumer to select the correct format:

```json:smallstair-core/package.json
{
  "name": "@smallstair/core",
  "version": "0.0.0",
  "main": "./dist/index.js",
...
Run `yarn build` to confirm compilation is working correctly. You should see a folder `smallstair-core/dist` which contains the compiled output.
...
Each file inside of `smallstair-core/src` is a component inside our design system. For example:

```tsx:smallstair-core/src/Button.tsx
...
```tsx:smallstair-core/src/index.tsx
import * as React from "react";
...
- Support using module path aliases like `@smallstair-core` for imports
- Write MDX for component documentation pages

For example, here's the included Story for our `Button` component:

```js:apps/docs/stories/button.stories.mdx
import { Button } from '@smallstair-core/src';
